...
{
    gpio_put(LCD_D0, (val >> 0) & 1);
    gpio_put(LCD_D1, (val >> 1) & 1);
    gpio_put(LCD_D2, (val >> 2) & 1);
    gpio_put(LCD_D3, (val >> 3) & 1);
    gpio_put(LCD_D4, (val >> 4) & 1);
    gpio_put(LCD_D5, (val >> 5) & 1);
    gpio_put(LCD_D6, (val >> 6) & 1);
    gpio_put(LCD_D7, (val >> 7) & 1);
...
void lcd_init(void)
{
    ...
    lcd_cmd(0x38);       // 8-bit, 2 lines, 5x8 font
...
{
    slot &= 0x07;                 // only 0..7 allowed
    lcd_cmd(0x40 | (slot << 3));  // set CGRAM address

    for (int i = 0; i < 8; i++)
        lcd_data(bitmap[i] & 0x1F);

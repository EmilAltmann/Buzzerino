...
typedef enum { OB_NONE=0, OB_CACTUS, OB_ROCK, OB_PTERO } ob_type_t;
...
typedef struct {
    obstacle_t obs[2];
    int spawn_in;       // ticks bis spawn
    int jump_ticks;     // >0 => Spieler oben
    int anim;           // 0/1
    int score;
    int game_over;
} game_t;
...
static int rand_range(int a, int b) { return a + (rng_u32() % (uint32_t)(b-a+1)); }
...
static void reset_game(game_t *g) {
    memset(g, 0, sizeof(*g));
    for (int i=0;i<2;i++) g->obs[i].active = 0;
...
static void spawn_obstacle(game_t *g) {
    int slot = -1;
    for (int i=0;i<2;i++) if (!g->obs[i].active) { slot=i; break; }
    if (slot < 0) return;
    ...
    // Typ wählen: unten häufiger, oben seltener
    uint32_t r = rng_u32() % 10;
    if (r < 5) { o->t = OB_CACTUS; o->row = 1; }
    else if (r < 8) { o->t = OB_ROCK; o->row = 1; }
    else { o->t = OB_PTERO; o->row = 0; }

    // Abstand variieren + mit Score etwas enger
    int min_gap = 6, max_gap = 12;
    if (g->score > 400) { min_gap = 5; max_gap = 10; }
    if (g->score > 800) { min_gap = 4; max_gap = 8;  }
    g->spawn_in = rand_range(min_gap, max_gap);
}

static void render(const game_t *g) {
    uint8_t row0[LCD_COLS], row1[LCD_COLS];
    for (int i=0;i<LCD_COLS;i++) { row0[i] = ' '; row1[i] = CH_GROUND; }
    ...
    // Score oben rechts (4 Stellen)
    char s[5];
    snprintf(s, sizeof(s), "%4d", g->score);
    for (int i=0;i<4;i++) row0[LCD_COLS-4+i] = (uint8_t)s[i];

    lcd_gotoxy(0,0); for (int i=0;i<LCD_COLS;i++) lcd_putc((char)row0[i]);
    lcd_gotoxy(1,0); for (int i=0;i<LCD_COLS;i++) lcd_putc((char)row1[i]);
}
...
static void render_game_over(int score) {
    ...
    // Score: xxxx (rechts ggf. mit Leerzeichen)
    // Beispiel: "Score: 0123    "
...
int main() {
    ...
    while (1) {
        bool up_now = gpio_get(SW_UP);               // Pullup: true=not pressed
        bool up_pressed = (prev_up && !up_now);      // edge
        prev_up = up_now;

        if (!g.game_over) {
            // Jump (nur wenn nicht schon in der Luft)
            if (up_pressed && g.jump_ticks == 0) {
                g.jump_ticks = 4; // 4 ticks oben
                buzzer_beep_ms(12);
            }

            // Animation / Physik
            g.anim ^= 1;
            if (g.jump_ticks > 0) g.jump_ticks--;

            // Spawning
            if (g.spawn_in > 0) g.spawn_in--;
            if (g.spawn_in == 0) spawn_obstacle(&g);

            // Move + collision (inkl. Vogel!)
            int player_row = (g.jump_ticks > 0) ? 0 : 1;
            for (int i=0;i<2;i++) {
                obstacle_t *o = &g.obs[i];
                if (!o->active) continue;

                // Kollision: gleicher x UND gleicher row
                // -> Ptero (row0) kollidiert nur beim Springen, Cactus/Rock (row1) nur am Boden

...
// "Subpixel"-Grid: 16 Spalten, 6 Reihen (2 LCD rows * 3 bands)
#define W 16
#define H 6

#define TICK_MS 180

typedef struct { uint8_t x, y; } pt_t;
...
static void make_3band_char(uint8_t mask3, uint8_t out[8]) {
    ...
    if (mask3 & 0x1) { out[0] = 0x1F; out[1] = 0x1F; } // top
    if (mask3 & 0x2) { out[3] = 0x1F; out[4] = 0x1F; } // mid
    if (mask3 & 0x4) { out[6] = 0x1F; out[7] = 0x1F; } // bot
}
...
static void load_subpixel_charset(void) {
    ...
    for (uint8_t m = 0; m < 8; m++) {
        ...
        lcd_create_char(m, bmp);   // slot m zeigt mask m (000..111)
    }
}

// ---------- framebuffer: 2 LCD rows x 16 cols, each cell = 3-bit mask ----------
...
// setzt genau EIN Band in einer existierenden Zeichenposition (OR), ohne andere zu löschen
static inline void fb_or_band(uint8_t x, uint8_t lcd_row /*0/1*/, uint8_t band /*0..2*/) {
    if (x >= W) return;
...
static int snake_len = 4; // Startlänge
static int score = 0;
...
static void snake_reset(void) {
    ...
    uint8_t sx = 6, sy = 2; // mittig, nach rechts
    for (int i = 0; i < snake_len; i++) {
        snake[i].x = sx + i;
        snake[i].y = sy;
    }
...
static int snake_step(dir_t dir) {
    ...
    // Selbstkollision:
    // - Wenn wir NICHT essen, wird der Tail entfernt -> Tail-Zelle darf "betreten" werden
    // - Wenn wir essen, bleibt Tail stehen -> Tail-Zelle zählt mit
    int end = snake_len;
    for (int i = 0; i < end; i++) {
        // bei normalem Move ignorieren wir i==0 (Tail), weil der gleich verschwindet
        if (!ate && i == 0) continue;
        if (snake[i].x == nh.x && snake[i].y == nh.y) return 0;
    }

    if (ate) {
        // Wachsen: KEIN Tail-Shift, nur neuen Kopf anhängen
        if (snake_len < SNAKE_MAX) {
            snake[snake_len] = nh;   // hinter den bisherigen Kopf
            snake_len++;
        } else {
            // wenn max, dann wie normal bewegen
        ...
        // normal bewegen: Tail weg, Kopf weiter
        memmove(&snake[0], &snake[1], sizeof(pt_t) * (snake_len - 1));
        snake[snake_len - 1] = nh;
    }
...
static void draw_frame(void) {
    ...
    // 2) Apfel als EIN Subpixel (Band) dazu-ORen
    //    Mapping: y=0..5 -> lcd_row=(y/3), band=(y%3)
    if (apple_active) {
        uint8_t lcd_row = (apple.y < 3) ? 0 : 1;
        uint8_t band    = (uint8_t)(apple.y % 3); // 0=top,1=mid,2=bot
...
// ---------- Interrupt-based input (Events + direction) ----------
...
static void dpad_irq(uint gpio, uint32_t events) {
    (void)events;
...
static void input_init_irqs(void) {
    gpio_init(SW_UP);    gpio_set_dir(SW_UP, GPIO_IN);    gpio_pull_up(SW_UP);
    gpio_init(SW_DOWN);  gpio_set_dir(SW_DOWN, GPIO_IN);  gpio_pull_up(SW_DOWN);
    gpio_init(SW_LEFT);  gpio_set_dir(SW_LEFT, GPIO_IN);  gpio_pull_up(SW_LEFT);
    gpio_init(SW_RIGHT); gpio_set_dir(SW_RIGHT, GPIO_IN); gpio_pull_up(SW_RIGHT);

    gpio_set_irq_enabled_with_callback(SW_UP, GPIO_IRQ_EDGE_FALL, true, &dpad_irq);
...
int main() {
    ...
    lcd_gotoxy(0,0); lcd_puts("Snake 16x6 apple");
    lcd_gotoxy(1,0); lcd_puts("Dpad directions");
    sleep_ms(900);

    absolute_time_t next = make_timeout_time_ms(TICK_MS);

    while (1) {
        // Richtung übernehmen (aus ISR), aber keine 180°-Wende
        dir_t req = g_dir_req;
        if (!is_opposite(dir, req)) dir = req;

        if (!snake_step(dir)) {
            show_game_over(score);

            // Events löschen, damit kein altes Event sofort restarten kann
            g_btn_events = 0;

            // warten auf UP-Event
            while ((g_btn_events & EV_UP) == 0) {
                sleep_ms(10);
            }
